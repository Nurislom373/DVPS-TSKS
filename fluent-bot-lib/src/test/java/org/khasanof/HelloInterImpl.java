...
public class HelloInterImpl implements HelloInterfaceHandler {
    ...
    @Override
    public void messageException(Update update, AbsSender sender) throws TelegramApiException {
        SendMessage message = new SendMessage(update.getMessage().getChatId().toString(), "Hello, Abror aka 2");
        sender.execute(message);
    }
