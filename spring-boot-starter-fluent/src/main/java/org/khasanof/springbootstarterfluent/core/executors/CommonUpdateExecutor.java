...
import org.khasanof.springbootstarterfluent.main.GenericSingleton;
...
public class CommonUpdateExecutor extends AbstractExecutor {
    ...
    public CommonUpdateExecutor(InvokerFunctions invokerFunctions,
        ...
        BreakerForEach.forEach(determination.determinationV2(update).stream(),
                ((entry, breaker) -> {
                    if (!FluentContext.updateExecutorBoolean.get()) {
                        invoker.invokeV2(invokerFunctions.fillAndGetV2(entry, update, instance));
                    } else {
                        breaker.stop();
                    }
